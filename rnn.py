...
class RNN:
    def __init__(self, sequence_length, num_classes, vocab_size, embedding_size,
                 cell_type, hidden_size, l2_reg_lambda=0.0, idx2Matrix, func = "lstm"):
        ...
        if func == "lstm":
            ...
            with tf.name_scope("bilstm"):
                fw_cell = self._get_cell(hidden_size, "lstm")
                bw_cell = self._get_cell(hidden_size, "lstm")
                cell = tf.nn.rnn_cell.DropoutWrapper(cell, output_keep_prob=self.dropout_keep_prob)
                all_outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell,
                                                cell_bw=bw_cell,
                                                inputs=self.embedded_chars,
                                                sequence_length=text_length,
                                                dtype=tf.float32)
